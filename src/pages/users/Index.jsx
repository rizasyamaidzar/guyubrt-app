...
const index = () => {
    ...
    const headers = ['Nama', 'No HP', "Status"];
    const fields = ['name', 'number_phone', "status"];
    const handleEdit = (id) => {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const result = await apiFetch(`/users/${id}`, 'DELETE'); // Use the helper
        ...
        <>
            <div className="w-3/4 mx-auto">
                ...
                <Table apiUrl={apiUrl} headers={headers} fields={fields} onEdit={handleEdit}
                    onDelete={handleDelete} detailUrl="/users/:id" />
            </div>
        </>
    )
}
